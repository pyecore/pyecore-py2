...
class ResourceSet(object):
    ...
    * ``resource_factory`` which is a factory used by the ResourceSet to build
    ...
    def remove_resource(self, resource):
        if not resource:
            return
        for key, value in dict(self.resources).iteritems():
    ...
    def resolve(self, uri, from_resource=None):
        ...
        uri_str, fragment = upath.rsplit('#', 1)
        if uri_str in self.resources:
...
class URI(object):
    ...
    def __split(self):
        if '://' in self.plain:
            self._protocol, rest = self.plain.split('://', 1)
        elif ':/' in self.plain:
            self._protocol, rest = self.plain.split(':/', 1)
        else:
    ...
    def _build_path_from(self, obj):
        ...
        if obj.eResource != self:
            ...
            if obj.eResource:
                ...
                for reguri, value in mm_registry.iteritems():
                    if value is root:
                        uri = reguri
                        break
                else:
